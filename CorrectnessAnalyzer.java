...
public class CorrectnessAnalyzer {
	...
	public List<?>[] analyzeAllMutants(SpecificationProvider specs, DataProvider dataSet) {
		...
		for (Method method : combinedMutantsClazz.getMethods()) {
			...
			if (method.getDeclaringClass().equals(combinedMutantsClazz)) {
				System.out.println("method in combinedMutantsClazz: "+method.getName());
		...
		Collections.shuffle(RMutants);
		usefulRSrAMutants[0] = RMutants;
		System.out.println("\n");
		
		System.out.println("Mutants that were selected by strict relative correctness test are -");
		count = 1;
		ArrayList<String> SrMutants = new ArrayList<String>();
		for (String mutant : strictlyRelativelyCorrectMutants) {
			System.out.println(count + ". " + mutant);
			SrMutants.add(mutant);
			count++;
		}
		Collections.shuffle(SrMutants);
		usefulRSrAMutants[1]= SrMutants;
		System.out.println("\n");
		System.out.println("Mutants that were selected by absolute correctness test are -");
		count = 1;
		ArrayList<String> AMutants = new ArrayList<String>();
		for (String mutant : absolutelyCorrectMutants) {
			System.out.println(count + ". " + mutant);
			AMutants.add(mutant);
			count++;
		}
		Collections.shuffle(AMutants);
		usefulRSrAMutants[2] = AMutants;
		System.out.println("\n\n\n########################################################################");
		/*
		 * test return value
		 * @author Jiarui Tian
